...
/*
  cgi-printarg.c -- CGI プログラムに対する引数を表示するプログラム
  ~yas/syspro/www/cgi-printarg.c
*/
...
void print_content() {
    ...
    printf("query_string=");
    safe_print_string(query_string);
    printf("\n");
    ...
    if (query_string == NULL) {
        ...
        for (i = 0; i < qc; i++) {
            char *decoded;
            printf("qv[%d]: ", i);
            safe_print_string(qv[i]);
            decoded = decode_url(qv[i]);
            printf("(");
            safe_print_string(decoded);
            printf(")\n");
